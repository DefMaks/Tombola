...
export default function RaffleDetailClient({ slug }) {
  ...
  // Silently track view in database for analytics (not displayed to users)
  useEffect(() => {
    if (!slug) return;
  ...
  // Sync shares count
  useEffect(() => {
    if (raffle && localSharesCount === null) {
      setLocalSharesCount(raffle.shares_count ?? 0);
    }
  }, [raffle, localSharesCount]);

  // Live polling for tickets_sold
  ...
  const sharesCount = localSharesCount ?? raffle.shares_count ?? 0;

  // Secondary gallery media images only
  ...
  return (
    <main className="max-w-lg mx-auto pb-32 min-h-screen">
        ...
        <div className="bg-card border border-border rounded-2xl p-4 shadow-2xl space-y-3">
          ...
          <div className="flex items-center justify-between text-xs pt-1 gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              {/* Share Counter Button */}
              <button
                type="button"
                onClick={() => setShareModalOpen(true)}
                className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/30 text-xs font-bold transition-all shadow-sm group"
                title="Partager ce round avec vos amis"
              >
      ...
      <ShareRoundModal
        ...
        onShareSuccess={() => {
          setLocalSharesCount(prev => (prev ?? 0) + 1);
        }}
      />
            ...
            <>
              ...
              <div className="space-y-5 pt-4">
                ...
                <p className="mt-4 text-[10px] text-center text-muted-foreground">
                  Les jeux d&apos;argent sont interdits aux mineurs. Jouez avec modération.
                </p>
              </div>
            </>
          )}
