...
const memDb = {
  ...
  advertisements: [
    ...
    },
    {
      ...
      description: 'Mid Page Banner Lokko',
      image_url: 'https://ucarecdn.com/6396e774-b7f5-4dbe-97de-85ffd257b3d7/-/preview/1000x384/',
      start_date: new Date(Date.now() - 86400000).toISOString(),
      end_date: new Date(Date.now() + 365 * 86400000).toISOString(),
      status: 'en cours',
    ...
    {
      id: 7,
      title: 'Annoncez vos produits sur Punchy',
      description: 'Footer Page Banner DefMaks',
      image_url: 'https://ucarecdn.com/3ce456eb-dde0-4acd-a56d-8dcc2cad8786/meetDefmaks.png',
      start_date: new Date(Date.now() - 86400000).toISOString(),
      end_date: new Date(Date.now() + 365 * 86400000).toISOString(),
      status: 'en cours',
  ...
  try {
    ...
    try {
      await client.query(`
        ...
        ALTER TABLE raffles ADD COLUMN IF NOT EXISTS views_count INT DEFAULT 0;
        ALTER TABLE raffles ADD COLUMN IF NOT EXISTS shares_count INT DEFAULT 0;
...
function runMockQuery(sql, params = []) {
  ...
  if (s.includes('UPDATE raffles SET views_count')) {
    const slug = params[0];
    const r = memDb.raffles.find((x) => x.slug === slug || x.id === slug);
    if (r) {
      r.views_count = (r.views_count || 0) + 1;
      return { rows: [{ views_count: r.views_count }] };
    }
    return { rows: [] };
  }

  if (s.includes('UPDATE raffles SET shares_count')) {
    const slug = params[0];
    const r = memDb.raffles.find((x) => x.slug === slug || x.id === slug);
    if (r) {
      r.shares_count = (r.shares_count || 0) + 1;
      return { rows: [{ shares_count: r.shares_count }] };
    }
    return { rows: [] };
  }
  ...
  if (s.includes('FROM advertisements')) {
    ...
    if (zone === 'void') return { rows: [] };
    const list = memDb.advertisements.filter((a) => a.zone === zone && a.is_active && a.zone !== 'void');
    return { rows: list };
  }
